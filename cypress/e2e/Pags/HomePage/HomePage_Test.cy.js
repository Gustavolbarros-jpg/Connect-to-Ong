describe('Testes da Página Inicial (HomePage)', () => {
    ...
    it('Ao iniciar a página, deve ser mostrada a Main Section e seus elementos internos.', () => {
        ...
        cy.contains('Conecte-se a ONGs e transforme sua pesquisa em impacto social.').should('be.visible');

        // Verifica se o parágrafo da Main Section está visível
        cy.contains('Conectamos universidades e ONGs em projetos sociais que desenvolvem soft skills e geram impacto real.').should('be.visible');
    ...
    it('Posteriormente, deve ser exibido a segunda section e seus elementos internos.', () => {
        ...
        cy.contains('Soluções de soft skills fortalecem competências.').should('be.visible');

        // Verifica se o primeiro elemento da section está visível
        cy.contains('Comunicação').should('be.visible');
        cy.contains('É a capacidade de transmitir e receber informações de forma clara, objetiva e respeitosa.').should('be.visible');

        // Verifica se o segundo elemento da section está visível
        cy.contains('Empatia').should('be.visible');
        cy.contains('É a habilidade de se colocar no lugar do outro, compreendendo seus sentimentos e perspectivas.').should('be.visible');

        // Verifica se o terceiro elemento da section está visível
        cy.contains('Colaboração').should('be.visible');
        cy.contains('Refere-se à capacidade de trabalhar bem em equipe, contribuindo com ideias, respeitando diferentes opiniões e buscando objetivos comuns.').should('be.visible');
    });
    ...
    it('E por último, a terceira section e seus elementos internos.', () => {
        ...
        cy.contains('Universidades se registram com seus interesses e áreas de atuação.').should('be.visible');

        // Verifica se o segundo elemento da section está visível
        cy.contains('Conexão').should('be.visible');
        cy.contains('A plataforma cruza os dados e sugere parcerias ideais.').should('be.visible');

        // Verifica se o terceiro elemento da section está visível
        cy.contains('Resultados').should('be.visible');
        cy.contains('Projetos finalizados geram certificados e impacto real na comunidade.').should('be.visible');
    });
