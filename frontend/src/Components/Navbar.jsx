...
function Navbar() {
  ...
  // Este useEffect ouve por mudanças no localStorage e atualiza o estado do componente.
  // Isso garante que o Navbar reaja ao logout, mesmo que aconteça em outra aba.
  useEffect(() => {
    const handleStorageChange = () => {
      console.log("Storage mudou, atualizando o estado do usuário.");
      try {
        const savedUser = localStorage.getItem("user");
  ...
  function handleLogout() {
    ...
    setUser(null); // Atualiza o estado local imediatamente
    navigate("/login");
  }
    ...
    <header className="bg-white fixed top-0 w-full z-50">
      <nav className="flex items-center justify-between px-5 py-7 font-semibold shadow-lg mx-auto">
        <div className="flex-shrink-0">
          <Link to={homePath} className="text-blue-600 font-bold text-[20px]">
            Connect To Ong
          </Link>
        </div>
          ...
          <ul className="text-[20px] font-bold flex flex-col items-center gap-4 pt-4">
            ...
            {user && (
              <li>
                <NavLink
                  to="/stepe-one"
                  className="text-white bg-blue-600 hover:bg-blue-800 font-bold py-2 px-5 rounded-[4px]"
                  onClick={() => setIsOpen(false)}
                >
                  Criar Projeto
                </NavLink>
          ...
          <div className="border-t border-gray-200 mt-4 pt-4 flex flex-col items-center gap-4">
            {user ? (
              ...
              <>
                <Link
                  to="/login"
                  className="text-blue-600 hover:text-blue-800 font-bold text-[20px]"
                  onClick={() => setIsOpen(false)}
                >
                  Acessar Conta
                </Link>
