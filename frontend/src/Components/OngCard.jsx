...
function OngCard({ ong, showActionButton, actionButtonText, onActionClick, isSelected }) {
    ...
    return (
        ...
        <div 
            className={`bg-white rounded-[4px] shadow-md p-6 flex flex-col justify-between h-auto border-2 transition-all duration-300 ${isSelected ? "border-blue-600 shadow-blue-200" : "border-transparent hover:shadow-xl"}`}
                ...
                {showActionButton && (
                    <div className="w-full sm:w-auto">
                        <button
                            ...
                            className={`w-full sm:w-auto bg-blue-600 text-white font-semibold py-2 px-5 rounded-[4px] transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 ${isSelected ? "bg-green-500 cursor-not-allowed" : "hover:bg-blue-700"}`}
                        >
