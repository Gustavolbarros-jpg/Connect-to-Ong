...
export default class ProjectControllers {
      ...
      if (result.ong_selecionada) {
        ...
        if (!isNaN(ongIdAsNumber)) {
          ...
          if (ong && ong.email && professor) {
            console.log(
              `ONG encontrada: ${ong.nome_ong}. Professor: ${professor.fullname}. Enviando e-mail para: ${ong.email}`
            );
            ...
            const htmlContent = `
                          <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #ddd; border-radius: 8px; overflow: hidden;">
                            ...
                            <div style="padding: 20px;">
                              <h2 style="color: #001449;">Olá, equipe da ${
                                ong.nome_ong || "Organização"
                              },</h2>
                              <p>Temos ótimas notícias! O professor(a) <strong>${
                                professor.fullname
                              }</strong> criou um novo projeto na plataforma <strong>Connect To ONG</strong> e gostaria de conectar-se com a sua organização para realizá-lo.</p>
                              ...
                              <div style="background-color: #f9f9f9; padding: 15px; border-radius: 5px; margin-top: 20px; border-left: 4px solid #0056b3;">
                                ...
                                <p><strong>Área de Interesse:</strong> ${
                                  result.area_interesse
                                }</p>
                                <p><strong>Descrição:</strong></p>
                                <p style="padding-left: 15px; border-left: 2px solid #ccc;"><em>${
                                  result.descricao_projeto
                                }</em></p>
                                
                                <h4 style="margin-top: 20px; color: #001449;">Detalhes Adicionais:</h4>
                                <ul style="list-style-type: none; padding: 0;">
                                  <li><strong>🎓 Alunos Envolvidos:</strong> ${
                                    result.quantidade_alunos
                                  }</li>
                                  <li><strong>💡 Habilidades a Desenvolver:</strong> ${
                                    result.soft_skills
                                  }</li>
                                  <li><strong>🕒 Carga Horária de Extensão:</strong> ${
                                    result.horas_extensao
                                  } horas</li>
                                </ul>
                              </div>
            ...
            console.log(
              `E-mail de notificação enviado com sucesso para a ONG ${ong.nome_ong}`
            );
          } else {
            let logMessage = "AVISO: Não foi possível enviar o e-mail. ";
            if (!ong)
              logMessage += "A ONG selecionada não foi encontrada no banco. ";
            if (!professor)
              logMessage +=
                "O professor criador do projeto não foi encontrado. ";
            if (ong && !ong.email)
              logMessage += "A ONG não possui um e-mail cadastrado. ";
  ...
  async subscribeUserToProject(userId, projectId) {
    try {
      ...
      const subscription = await this.dataAccess.subscribeUser(
        userId,
        projectId
      );
