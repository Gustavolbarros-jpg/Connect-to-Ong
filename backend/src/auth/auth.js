  ...
  try {
    ...
    if (!fullname || !email || !password || !institution)
      return res
        .status(400)
        .send({
          message: "Nome, e-mail, instituição e senha são obrigatórios.",
        });

    if (password.length < 8)
      return res
        .status(400)
        .send({ message: "A senha precisa ter no mínimo 8 caracteres." });
    ...
    if (existingUser)
      return res
        .status(409)
        .send({ message: "Um usuário com este e-mail já existe." });
...
authRouter.post("/login", (req, res) => {
  passport.authenticate("local", { session: false }, (error, user, info) => {
    ...
    if (!user)
      return res
        .status(401)
        .send({ message: info.message || "Credenciais inválidas." });
